...
| arch/riscv_gcc/common/{riscv_insn.h,core_sil.h} | mod (c3-port) | A 拡張（アトミック命令）を持たない RV32IMC 向けの test-and-set 経路を追加（`riscv_tas_uint32` / `TOPPERS_test_and_assign` の両方）。ESP32-C3 は `amoswap` も `lr/sc` も**命令として存在しない**ため、既存 2 経路（`USE_RISCV_LLSC` の lr/sc と amoswap）はどちらもアセンブルできない。`#if !defined(__riscv_atomic)`（-march に 'a' が無いときだけ GCC が未定義にするマクロ）で分岐し、mstatus.MIE を落としたうえでの「読んで、0 なら書く」で代替する。**単一プロセッサでのみ正しい**ので `TNUM_PRCID >= 2` は `#error` で止める。MIE のビット値は riscv.h の `MSTATUS_MIE` と同値だが、両ヘッダとも riscv.h を include しないため、この経路の中でローカル定数として持つ（他ターゲットの include 依存を増やさないため）。実測: 既存チップ（A 拡張あり）は分岐に入らず生成コード不変——C6 の minimal ステージ再ビルドで banner.o（`__DATE__`/`__TIME__`）以外バイト同一。2026-09-23 | 未 |
